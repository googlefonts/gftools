...
def main(args=None):
    ...
    subparsers = parser.add_subparsers(dest="command", required=True, metavar='"build" or "dump"')    
    build_parser = subparsers.add_parser("build", parents=[universal_options_parser], help="Add a debg table to a font which contains the build's dependencies")
    ...
    dump_parser = subparsers.add_parser("dump", parents=[universal_options_parser], help="Dump a font's dependencies")
    dump_parser.add_argument("-o", "--out", help="output a requirements file to specified path")
