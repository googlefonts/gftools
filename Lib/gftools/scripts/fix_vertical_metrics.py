...
class TextMetricsView(object):
    ...
    def add_metric(self, font_name, vmet):
        ...
        typo_total = (
            vmet.ascents.os2typo + abs(vmet.descents.os2typo) + vmet.linegaps.os2typo
        )
    ...
    def print_warnings(self):
        if self._inconsistent:
            ...
            print(
                _.format(" ".join([str(x) for x in self._inconsistent])),
                end="\n\n",
                file=self.outstream,
            )
    ...
    def print_inconsistent_table(self):
        ...
        for metrickey, row in self._inconsistent_table.items():
            ...
            for k, r in tbl.items():
                print(
                    "WARNING: Inconsistent %s:" % k, ", ".join(r), file=self.outstream
                )
...
# ascent parameters
parser.add_argument("-a", "--ascents", type=int, help=("Set new ascents value."))
...
parser.add_argument(
    ...
    type=int,
    help=(
        "Set new ascents value in 'Horizontal Header'"
...
# descent parameters
parser.add_argument("-d", "--descents", type=int, help=("Set new descents value."))
parser.add_argument(
    "-dh",
    "--descents-hhea",
    type=int,
    help=(
        "Set new descents value in 'Horizontal Header'"
        " table ('hhea'). This argument"
        " cancels --descents."
    ),
)
...
# linegaps parameters
parser.add_argument("-l", "--linegaps", type=int, help=("Set new linegaps value."))
parser.add_argument(
    "-lh",
    "--linegaps-hhea",
    type=int,
    help=("Set new linegaps value in 'Horizontal Header'" " table ('hhea')"),
)
parser.add_argument(
    "-lt",
    "--linegaps-typo",
    type=int,
    help=("Set new linegaps value in 'Horizontal Header'" " table ('OS/2')"),
)
...
def main(args=None):
    ...
    if (
        ...
        or options.descents
        or options.linegaps
        or options.linegaps == 0
        or options.ascents_hhea
        or options.ascents_typo
        or options.ascents_win
        or options.descents_hhea
        or options.descents_typo
        or options.descents_win
        or options.linegaps_hhea
        or options.linegaps_hhea == 0
        or options.linegaps_typo
        or options.linegaps_typo == 0
    ):
        for f in fonts:
            try:
