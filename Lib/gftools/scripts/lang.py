...
# UDHR
UHDR_URL_TEMPLATE = (
    "https://www.un.org/{language_code}/universal-declaration-human-rights/"
)
...
flags.DEFINE_boolean(
    "overwrite_sample_text",
    False,
    "Whether to overwrite sample text with any generated from UDHR translations",
    short_name="w",
)
flags.DEFINE_bool(
    "gen_lang", False, "Whether to generate the lang metadata package", short_name="l"
)
flags.DEFINE_bool(
    "report",
    False,
    "Whether to output a report of lang metadata insights",
    short_name="r",
)
flags.DEFINE_string("noto_root", None, "Path to noto root", short_name="n")
flags.DEFINE_bool(
    "mark_historical_languages",
    False,
    "Whether to update the lang metadata package to specify historical scripts",
    short_name="h",
)
...
class Cldr:
    ...
    def __init__(self):
        ...
        (
            lang_names_supplement,
            region_names_supplement,
        ) = self._ParseDisplayNamesFromSupplementalData()
        lang_names.update(lang_names_supplement)
        region_names.update(region_names_supplement)
        lang_names.update(
            {
                o: LANGUAGE_OVERRIDES[o]["name"]
                for o in LANGUAGE_OVERRIDES
                if "name" in LANGUAGE_OVERRIDES[o]
            }
        )
        script_names.update(
            {
                s: SCRIPT_OVERRIDES[s]["name"]
                for s in SCRIPT_OVERRIDES
                if "name" in SCRIPT_OVERRIDES[s]
            }
        )
        (
            region_populations,
            lang_populations,
            lang_regions,
        ) = self._ParseLanguageRegionInfo()
        region_groups = self._ParseRegionGroups()

        self.langs = self._CompileLanguages(
            lang_index,
            lang_historical,
            lang_aliases,
            lang_names,
    ...
    def _ParseLanguageIndex(self):
        root = etree.parse(
            os.path.join(self._zip_dir.name, SUPPLEMENTAL_DATA_XML_PATH),
            parser=NO_COMMENTS_XML_PARSER,
        )
    ...
    def _ParseHistoricalLanguages(self):
        ...
        for l in language_data:
            ...
            if l.get("alt") == "secondary":
                l_id = l.get("type")
                scripts = (
                    [] if l.get("scripts") is None else l.get("scripts").split(" ")
                )
    ...
    def _ParseLanguageAliases(self):
        root = etree.parse(
            os.path.join(self._zip_dir.name, SUPPLEMENTAL_METADATA_XML_PATH),
            parser=NO_COMMENTS_XML_PARSER,
        )
    ...
    def _ParseDisplayNamesFromEnXml(self):
        root = etree.parse(
            os.path.join(self._zip_dir.name, Cldr._GetLangXmlPath(EN)),
            parser=NO_COMMENTS_XML_PARSER,
        )
        ...
        lang_names = {
            lang.get("type"): lang.text
            for lang in languages_data
            if lang.get("alt") != "long"
        }
    ...
    def _ParseDisplayNamesFromSupplementalData(self):
        root = etree.parse(os.path.join(self._zip_dir.name, SUPPLEMENTAL_DATA_XML_PATH))
    ...
    def _ParseLanguageRegionInfo(self):
        ...
        for region_data in regions_data:
            ...
            for lang_population_data in region_data:
                ...
                lang_populations[lang_code] += (
                    region_population * population_percent / 100
                )
    ...
    def _ParseRegionGroups(self):
        ...
        raw_groups = {
            group_data.get("type"): group_data.get("contains").split(" ")
        ...
        for lang_code in lang_index:
            ...
            for script_code in lang_index[lang_code]:
                ...
                if key in lang_names:
                    ...
                    display_name = "{lang}, {script}".format(
                        lang=lang_names[lang_code], script=script_names[script_code]
                    )
                else:
    ...
    def _CompileRegions(self, region_names, region_populations):
        ...
        for region_code in region_populations:
            ...
            assert display_name is not None, (
                "Missing display name for region: " + region_code
            )
    ...
    def _CompileScripts(self, lang_index, script_names):
        script_codes = {
            script for lang_code in lang_index for script in lang_index[lang_code]
        }
        return {
            script_code: script_names[script_code]
            for script_code in script_codes
            if script_code in script_names
        }
            ...
            if lang is None:
                ...
                for l in self.langs.values():
                    if l.lang_code == lang_code:
                        print(l)
                warnings.warn(
                    "Unable to find language when setting region info: " + lang_code
                )
                continue
    ...
    class Language:
        ...
        def SetExemplarChars(self, exemplar_chars):
            assert self.exemplar_chars is None, (
                "Exemplar chars already set on language: " + self.id
            )
...
class UdhrTranslations:
    ...
    def __init__(self):
        ...
        for udhr in self._udhrs:
            ...
            if udhr.iso639_3 != "yue":
                continue
            key = udhr.iso639_3 + "_" + udhr.iso15924
            if key in self._udhr_map and self._udhr_map[key].stage > udhr.stage:
                w = 'Skipping UDHR "{other}" in favor of "{this}"'.format(
                    other=self._udhr_map[udhr.key].name, this=udhr.name
                )
                warnings.warn(w)
                continue
...
def _GetHyperglotLanguage(lang, hyperglot_languages):
    ...
    if (
        lang_code in SUPPLEMENTAL_ALIASES
        and SUPPLEMENTAL_ALIASES[lang_code] in hyperglot_languages
    ):
...
def _GetPreferredName(lang, hg_lang):
    if (
        lang.id in LANGUAGE_OVERRIDES
        and "preferred_name" in LANGUAGE_OVERRIDES[lang.id]
    ):
...
def _GetExemplarCharacters(cldr, lang, hg_lang):
    ...
    if text_format.MessageToString(exemplar_chars) == text_format.MessageToString(
        fonts_public_pb2.ExemplarCharsProto()
    ):
        return None
...
def _WriteReport(out_dir):
    ...
    for lang in languages.values():
        row = [
            ...
            or not lang.sample_text.HasField("fallback_language")
            else lang.sample_text.fallback_language,
            ""
            if not lang.HasField("sample_text")
            or not lang.sample_text.HasField("fallback_language")
            else languages[lang.sample_text.fallback_language].name
            or languages[lang.sample_text.fallback_language].id,
        ]
        rows.append(row)
    ...
    if FLAGS.noto_root is not None:
        ...
        for fontfile in glob.iglob(os.path.join(FLAGS.noto_root, "*-Regular.ttf")):
            ...
            if len(supported) == 0:
                ...
                for lang in languages.values():
                    if lang_hint in re.sub(r"[^\w]+", "", lang.name).lower():
                        ...
                        lang.script in scripts
                        and lang_hint
                        in re.sub(r"[^\w]+", "", scripts[lang.script].name).lower()
                    ):
                        supported.append(lang)
            if len(supported) == 0:
            ...
            [
                supported_without_sample_text.append(l.name or l.id)
                for l in supported
                if not l.HasField("sample_text")
                and l not in supported_without_sample_text
            ]

        rows = [
            [
                "Notos without lang support",
                "Notos without sample text",
                "Supported language without sample text",
            ]
        ]
        for i in range(
            max(
                len(notos_without_lang),
                len(notos_without_sample_text),
                len(supported_without_sample_text),
            )
        ):
            row = [
                "" if i >= len(notos_without_lang) else notos_without_lang[i],
                ""
                if i >= len(notos_without_sample_text)
                else notos_without_sample_text[i],
                ""
                if i >= len(supported_without_sample_text)
                else supported_without_sample_text[i],
            ]
            rows.append(row)
    ...
    with Cldr() as cldr:
        for lang in langs.values():
            ...
            if historical:
                ...
                _WriteProto(
                    lang, os.path.join(FLAGS.out, "languages", lang.id + ".textproto")
                )
