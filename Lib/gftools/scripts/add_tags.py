...
def main(args=None):
    ...
    parser.add_argument(
        "source",
        type=Path,
        help="Source CSV file (rows to merge from)"
    )
    parser.add_argument(
        "target",
        type=Path,
        help="Target CSV file (rows to merge into)"
    )
    parser.add_argument(
        "tag_category",
        help="Tag category to add/replace"
    )
    parser.add_argument(
        "-o", "--output",
        type=Path,
        help="Output CSV file (default: overwrite target)"
    )
