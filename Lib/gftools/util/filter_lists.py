...
def get_namelist_for_filterlist(filterlistFilename):
    ...
    if markerDir != FILTER_LISTS_DIR_NAME:
        raise MissingCharsetDirectory(
            'charset directory not found in "{path}".'.format(path=dirname)
        )
...
def get_name_by_name(search_name, production_name=False):
    ...
    """
    entry = (None, None)
    glyph = GlyphData.by_name.get(search_name, None) or GlyphData.by_prodname.get(
        search_name, None
    )
    if glyph is not None:
        entry = (glyph.name, glyph.production_name)
...
def get_unicode_by_name(name):
    glyph = GlyphData.by_name.get(name, None) or GlyphData.by_prodname.get(name, None)
...
def translate_name(name, production_name=False):
    ...
    if codepoint is not None:
        new_name = (
            get_name_by_unicode(codepoint, production_name=production_name)
            if codepoint is not None
            else None
        )
        if new_name is not None:
    ...
    if "_" in name:
        return "_".join(
            translate_name(component, production_name=production_name)
            for component in name.split("_")
        )
    if "." in name:
        basename, extension = name.split(".", 1)
        return ".".join(
            [translate_name(basename, production_name=production_name), extension]
        )
...
def check_filterlist_in_namelist(filterListFileName, namelistCache=None):
    ...
    if not namelistFilename:
        return (
            False,
            "No Namelist file found for {filterlist}".format(
                filterlist=filterListFileName
            ),
            None,
        )
    ...
    useProductionNames = (
        "uni names" in filterListFileName or "uni-names" in filterListFileName
    )
    ...
    if len(missingChars):
        names = ", ".join(["0x{0:04X} {1}".format(c, name) for c, name in missingChars])
        message.append(
            "Unicode chars not in Namelist but in filter list:"
    ...
    if len(missingNoncodes):
        if useProductionNames:
            missingNoncodes = [
                "{0} as {1}".format(noncode, noncodes2prodcodes[noncode])
                for noncode in missingNoncodes
            ]
        message.append(
            "None-Unicode not in Namelist but in filter list:"
...
def check_filterlist_equals_namelist(filterlist, namelistFilename, namelistCache=None):
    ...
    if codepoints_set != namelist["ownCharset"]:
        ...
        if len(codepoints_not_in_namelist):
            message.append(
                "Unicode in filter list missing in Namelist:\n{0}".format(
                    ", ".join(
                        formatCodePoints(c) for c in sorted(codepoints_not_in_namelist)
                    )
                )
            )
        ...
        if len(codepoints_not_in_filterlist):
            message.append(
                "Unicode in Namelist missing in filter list:\n{0}".format(
                    ", ".join(
                        formatCodePoints(c)
                        for c in sorted(codepoints_not_in_filterlist)
                    )
                )
            )
    ...
    if noncodes_set != namelist["ownNoCharcode"]:
        ...
        if len(noncodes_not_in_namelist):
            message.append(
                "Unencoded chars in filter list missing in Namelist:\n{0}".format(
                    ", ".join(sorted(noncodes_not_in_namelist))
                )
            )
        noncodes_not_in_filterlist = namelist["ownNoCharcode"] - noncodes_set
        if len(noncodes_not_in_filterlist):
            message.append(
                "Unencoded in Namelist missing in filter list:\n{0}".format(
                    ", ".join(sorted(noncodes_not_in_filterlist))
                )
            )
    ...
    if len(message):
        message.insert(
            0,
            "Namelist and Filter-List are out of sync."
...
def _build_filterlists_equal_namelists(filterlist, namelistFilename):
    ...
    def test_filterlist_equals_namelist(self):
        passed, message, namelist = check_filterlist_equals_namelist(
            filterlist, namelistFilename, self._cache
        )
        if passed:
            return
        self.assertTrue(passed, msg=message)
...
def _build_friendly_names_production_names_equal(
    pathparts, prod_names_file, nice_names_file
):
    test_name = "test_nice_names_uni_names_equal {0}".format(
        "{marker dir}".join(pathparts)
    )
    ...
    def test_friendly_names_production_names_equal(self):
        ...
        nice_prod_names = [translate_name(name) for name in prod_names]
        ...
        self.assertTrue(
            dupes_in_prod_names == 0 and dupes_in_nice_names == 0,
            msg="\n".join(message),
        )
        ...
        if len(not_in_prod):
            log_message(
                "Entries in nice names but not in uni names(#{1}):\n{0}\n".format(
                    ", ".join(not_in_prod), len(not_in_prod)
                )
            )

        if len(not_in_nice):
            nice2prod = dict(zip(nice_prod_names, prod_names))
            items = ",".join("{0} as {1}".format(nice2prod[n], n) for n in not_in_nice)
            log_message(
                "Entries in uni names but not in nice names (#{1}):\n{0}\n".format(
                    items, len(not_in_nice)
                )
            )
...
def build_friendly_names_production_names_equal(files):
    ...
    nice_names_parts = {
        tuple(f.split(nice_names_dir, 1)) for f in files if nice_names_dir in f
    }
    prod_names_parts = {
        tuple(f.split(prod_names_dir, 1)) for f in files if prod_names_dir in f
    }
    ...
    for pathparts in matches:
        ...
        yield _build_friendly_names_production_names_equal(
            pathparts, prod_names_file, nice_names_file
        )
...
def initTestProperties(cls, files):
    ...
    for test_generator in (
        build_filterlists_in_namelists,
...
def main(args):
    if len(args) < 2:
        raise Exception(
            "The first argument must be the search directory" " for nam files."
        )
    searchDirectory = args[1]
    files = subprocess.check_output(
        ["find", searchDirectory, "-type", "f", "-path", "*/filter lists/*.txt"]
    ).decode("utf-8")
